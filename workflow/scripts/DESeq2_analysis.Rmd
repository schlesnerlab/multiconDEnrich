...
knitr::opts_chunk$set(
  ...
  fig.width = 7, fig.height = 7, dpi = 300 
)
...
ggsave(
  filename = file.path(plot_path, "Volcano_plot.png"),
  plot = volcano_plot
)
...
```{r msigdb, fig.width = 7,fig.height =7, results='asis'}
  ...
  if (!is.null(enrich_res)) {
    if (nrow(enrich_res) > 1) {
      ...
      if (conf[["gsea"]][[gsea_name]][["use_gsea"]]) {
        ...
        )), plot = enrich_plots,fix_text_size = FALSE)
        print(enrich_plots)
        ...
        ggsave(
          ...
          cnetplot, width = 16, height = 16, fix_text_size = FALSE
        )
        all_plots <- cnetplot
      } else {
        enrich_plots <- NULL
